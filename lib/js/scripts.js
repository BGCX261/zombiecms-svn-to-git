...
function validate(_form)
{
	switch(_form)
	{
		case "add_root":
			...
			//input map: username, pass, retype pass, email, SUBMIT BUTTON
...
function init_terminal()
{
	...
	Event.observe($("exec_query"), "click", function() {
		if(old_query != "" || $F("query").length < old_query.length)
		{
			if(old_query.match(/>>/))
				var new_query = "query="+$F("query").substr(1,$F("query").length);
			else
				var new_query = "query="+$F("query").substr(old_query.length, $F("query").length);
		}
		else new_query = "query="+$F("query");
		...
		if(really != false) {
			$("console").request({parameters: new_query, onComplete: function(transport){
					$("query").value += transport.responseText + ">";
					old_query = $F("query");
					$("query").focus();
			}});
		}
