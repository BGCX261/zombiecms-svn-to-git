...
class Blog
{
	...
	function del($id)
	{
		$del_article = mysql_query("DELETE FROM articles WHERE id = '$id'");
		if($del_article) return true;
		return false;
	}
	function purge($id){
		$del_article = mysql_query("DELETE FROM articles WHERE id='$id'");
		$del_stats = mysql_query("DELETE FROM article_stats WHERE article_id = '$id'");
		$del_meta = mysql_query("DELETE FROM article_meta WHERE article_id = '$id'");
		$del_comments = mysql_query("DELETE FROM article_comments WHERE article_id = '$id'");
		if($del_article && $del_stats && $del_comments && $del_meta) return true;
		return false;
	}
}
?>